...
export class MeetingSchedulingService {
  ...
  public findOptimalMeetingTimes(
    ...
    const conflicts = this.findConflictingMeetings(
      suggestions,
      existingMeetings,
    );
  ...
  private getSearchPeriod(request: SchedulingRequest): {
    start: Date;
    end: Date;
  } {
    const now = new Date();
    const start =
      request.earliestDate || new Date(now.getTime() + 24 * 60 * 60 * 1000); // Tomorrow
    const end =
      request.latestDate || new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000); // 2 weeks
  ...
  private findSuitableSlots(
    ...
    for (
      let i = 0;
      i <= window.availableSlots.length - Math.ceil(requiredDuration / 30);
      i++
    ) {
      const startSlot = window.availableSlots[i];
      const endTime = new Date(
        startSlot.start.getTime() + requiredDuration * 60 * 1000,
      );

      // Check if we have enough consecutive slots
      if (
        this.hasConsecutiveSlots(window.availableSlots, i, requiredDuration)
      ) {
  ...
  private selectBestSuggestion(
    suggestions: SchedulingSuggestion[],
  ): SchedulingSuggestion | undefined {
  ...
  private generateSummary(
    ...
    const optimal = suggestions.filter(
      (s) => s.score >= 80 && s.conflicts.length === 0,
    ).length;
  ...
  public validateMeetingRequest(request: SchedulingRequest): string[] {
    ...
    if (
      ...
      request.latestDate &&
      request.earliestDate >= request.latestDate
    ) {
