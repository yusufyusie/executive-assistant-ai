...
export class GeminiService {
  ...
  private getIntelligentMockResponse(request: GeminiRequest): GeminiResponse {
    ...
    if (prompt.includes('task') || prompt.includes('todo')) {
      if (prompt.includes('create') || prompt.includes('add')) {
        return {
          text: 'I can help you create a new task. Based on your request, I suggest:\n\n' +
      ...
      if (prompt.includes('prioritize') || prompt.includes('priority')) {
        return {
          text: 'I\'ll analyze your tasks and provide intelligent prioritization based on:\n\n' +
    ...
    if (prompt.includes('meeting') || prompt.includes('schedule') || prompt.includes('calendar')) {
      if (prompt.includes('schedule') || prompt.includes('book')) {
        return {
          text: 'I can help you schedule a meeting. I\'ll analyze:\n\n' +
      ...
      if (prompt.includes('conflict') || prompt.includes('available')) {
        return {
          text: 'I\'ll check for scheduling conflicts and availability. Based on your calendar:\n\n' +
    ...
    if (prompt.includes('email') || prompt.includes('send') || prompt.includes('message')) {
      if (prompt.includes('draft') || prompt.includes('compose')) {
        return {
          text: 'I can help you compose a professional email. I\'ll consider:\n\n' +
      ...
      if (prompt.includes('follow up') || prompt.includes('reminder')) {
        return {
          text: 'I\'ll help you create an effective follow-up email:\n\n' +
    ...
    if (prompt.includes('help') || prompt.includes('assist')) {
      return {
        text: 'I\'m your AI Executive Assistant, ready to help with:\n\n' +
              '📅 **Calendar Management**\n' +
    ...
    return {
      text: 'I understand you\'re looking for assistance. As your AI Executive Assistant, I can help with:\n\n' +
  ...
  private getMockResponse(prompt: string): GeminiResponse {
    return {
      text: `I understand your request: "${prompt.substring(0, 100)}..."\n\n` +
  ...
  async analyzeIntent(text: string): Promise<{
    ...
    if (lowerText.includes('create') && (lowerText.includes('task') || lowerText.includes('todo'))) {
      return {
        intent: 'create_task',
        confidence: 0.9,
  ...
  private extractMeetingEntities(text: string): Record<string, any> {
    ...
    if (durationMatch) {
      ...
      entities.duration = unit.startsWith('hour') || unit.startsWith('hr') ? value * 60 : value;
    }
  ...
  private extractEmailEntities(text: string): Record<string, any> {
    ...
    else if (text.toLowerCase().includes('important')) entities.priority = 'high';
